...
public class DemoActivityServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		for(int monthNumber = 8; monthNumber<11; monthNumber++){
			...
			for(int i=0;i<30;i++){
				...
				for(int j=8;j<22;j+=5){
					...
					Date s1 = cal.getTime();
					int randDuration = new Random().nextInt(4*60*59);
					cal.add(Calendar.SECOND, randDuration);
					Date e1 = cal.getTime();
					
					int freeTimeOrStudyActivity = new Random().nextInt(4);
					...
					if(freeTimeOrStudyActivity != 0)
					{
						...
						if(newType.equals("Study")){
							
						randPages = (long) new Random().nextInt(randDuration)/180;
